...
import cn.edu.zut.bigdata.sparkstreaming.SparkStreamingUpdate.{ssc, words}
...
object SparkStreamingFromKafka extends App {
  ...
  val kafkaParams = Map[String,Object](
    ...
    //创建消费者组
    ConsumerConfig.GROUP_ID_CONFIG->"CountShop",
    //自动移动到最新的偏移量
  ...
  val count = dStream.map(record=>{
    ...
    (strs(4).replace("[",""),strs(8),strs(11))
  }).filter(t=>{
    if(t._3.equals("200") && t._2.startsWith("shop/")) true
    else false
  }).map(t=>{
    (t._1+":"+t._2.replace("shop/","").replace(".html",""),1)
  });
